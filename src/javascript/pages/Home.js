...
const Home = () => {
  ...
  let scrollModule = useMemo(()=>new SmoothScrollModule(),[]);

  useEffect(() => {
    scrollModule.setupSmoothScroll(document.querySelector('.js-scroll-container'));
  }, []);
        ...
        {({state}) => (
          <section className="js-scroll-container">
            ...
            {state.themeList.map((theme, index) => 
              <section onClick={() => handleClick(theme)}>
